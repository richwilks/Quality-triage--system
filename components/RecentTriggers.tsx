      ...
      <details open>
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs text-deck-dim">
            The last 10 {actionFilter === 'ALL' ? 'signals' : `${actionFilter === 'BUY' ? 'buy' : 'sell'} signals`}{' '}
            across your whole watchlist, newest first - both watch and confirmed.
          </p>
              ...
              <tbody>
                {triggers.map((t, idx) => (
                  <tr key={idx} className="border-t border-deck-border">
                    ...
                    <td
                      className={`py-1.5 pr-3 font-semibold ${t.action === 'BUY' ? 'text-emerald-700' : 'text-red-700'}`}
                    >
                      {t.action}
                    </td>
