...
const TABS = [
  {
    ...
    label: 'Home',
    icon: (active: boolean) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? '#2A6F77' : '#94A3B8'} strokeWidth="2">
        ...
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab.icon(active)}
              <span
                className={`text-[10px] font-medium ${active ? 'text-brand-primary' : 'text-slate-400'}`}
              >
                {tab.label}
              </span>
            </Link>
