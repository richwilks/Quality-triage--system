...
// Zoom presets - trading-day counts from the end of the series. "1Y" covers
// the whole fetched window (history/route.ts fetches 1 year), so there's no
// separate "All" option - it'd be identical.
...
// Draws only the [rangeStart, rangeEnd] slice (inclusive) - a null value or a
// point outside the range breaks the line, same as a genuine data gap.
function buildPath(
  values: (number | null)[],
...
export default function StockChart({ history }: { history: StockHistory }) {
  ...
  // Zoom: which trailing window of the already-fetched data is visible.
  // Only affects rendering/scaling, not which indicators were computed - a
  // zoomed-in view also auto-rescales the y-axis to that window's own
  // min/max, which is what actually makes zooming useful for reading detail
  // rather than just stretching the same pixels.
  const [rangeDays, setRangeDays] = useState<number>(Infinity)
  const effectiveDays = Math.min(rangeDays, n)
  const visibleStart = n > 0 ? Math.max(0, n - effectiveDays) : 0
  const visibleEnd = n > 0 ? n - 1 : 0

  // Indicator visibility toggles - click a legend chip to show/hide it.
  ...
  const xScale = (i: number) =>
    visibleEnd <= visibleStart
      ? MARGIN.left
      : MARGIN.left + ((i - visibleStart) / (visibleEnd - visibleStart)) * (plotRight - MARGIN.left)

  const priceBottom = PRICE_H - PRICE_BOTTOM
  const { min: priceMin, max: priceMax } = useMemo(() => {
    const slice = <T,>(arr: T[]) => arr.slice(visibleStart, visibleEnd + 1)
    const values = [
      ...slice(close),
      ...(showSma50 ? slice(sma50) : []),
      ...(showSma200 ? slice(sma200) : []),
    ].filter((v): v is number => v != null)
    const lo = Math.min(...values)
    const hi = Math.max(...values)
    const pad = (hi - lo) * 0.08 || 1
    return { min: lo - pad, max: hi + pad }
  }, [close, sma50, sma200, visibleStart, visibleEnd, showSma50, showSma200])
  ...
  const { min: macdMin, max: macdMax } = useMemo(() => {
    const slice = <T,>(arr: T[]) => arr.slice(visibleStart, visibleEnd + 1)
    const values = [...slice(macd.macdLine), ...slice(macd.signalLine)].filter((v): v is number => v != null)
    if (values.length === 0) return { min: -1, max: 1 }
    const lo = Math.min(...values, 0)
    const hi = Math.max(...values, 0)
    const pad = (hi - lo) * 0.1 || 1
    return { min: lo - pad, max: hi + pad }
  }, [macd, visibleStart, visibleEnd])
  ...
  function indexFromClientX(svg: SVGSVGElement | null, clientX: number): number {
    if (!svg || n === 0) return visibleStart
    const rect = svg.getBoundingClientRect()
    const frac = (clientX - rect.left) / rect.width
    const viewX = frac * VIEW_W
    const t = (viewX - MARGIN.left) / (plotRight - MARGIN.left)
    const idx = Math.round(visibleStart + t * (visibleEnd - visibleStart))
    return Math.min(visibleEnd, Math.max(visibleStart, idx))
  }

  function selectRange(days: number) {
    setRangeDays(days)
    setHovered(null) // avoid a stale crosshair position from the previous window
  }
  ...
  const inVisibleRange = (i: number) => i >= visibleStart && i <= visibleEnd
  const smaSignals = signals.filter(
    (s) => s.strategy === 'SMA_CROSSOVER' && inVisibleRange(s.index) && (showSma50 || showSma200)
  )
  const rsiSignals = signals.filter((s) => s.strategy === 'RSI' && inVisibleRange(s.index))
  const macdSignals = signals.filter((s) => s.strategy === 'MACD' && inVisibleRange(s.index))
  ...
  return (
    <div className="space-y-1">
      {/* Zoom - a trailing window of the already-fetched year of data;
          rescales both axes to that window, doesn't refetch anything. */}
        ...
        {RANGE_PRESETS.map((preset) => (
          <button
            ...
            className={`rounded px-2 py-0.5 text-xs font-medium ${
              effectiveDays === Math.min(preset.days, n)
                ? 'bg-deck-accent text-white'
                : 'bg-deck-raised text-deck-body hover:bg-deck-border'
            }`}
          >
            {preset.label}
          </button>
      ...
      {/* Legend - identity channel for the line series; SMA/RSI/MACD chips
          are clickable to show/hide that indicator. */}
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-deck-body">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded px-1 py-0.5 hover:bg-deck-raised ${showSma50 ? '' : 'opacity-40'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded px-1 py-0.5 hover:bg-deck-raised ${showSma200 ? '' : 'opacity-40'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded px-1 py-0.5 hover:bg-deck-raised ${showRSI ? '' : 'opacity-40'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded px-1 py-0.5 hover:bg-deck-raised ${showMACD ? '' : 'opacity-40'}`}
        >
      ...
      <div className="relative">
        ...
        >
          ...
          {showSma200 && (
            <path
              d={buildPath(sma200, xScale, priceYScale, visibleStart, visibleEnd)}
              fill="none"
              stroke={COLOR.sma200}
              strokeWidth={2}
          ...
          {showSma50 && (
            <path
              d={buildPath(sma50, xScale, priceYScale, visibleStart, visibleEnd)}
              fill="none"
              stroke={COLOR.sma50}
              strokeWidth={2}
              strokeLinejoin="round"
          ...
          <path
            d={buildPath(close, xScale, priceYScale, visibleStart, visibleEnd)}
            fill="none"
            stroke={COLOR.close}
            strokeWidth={2}
            strokeLinejoin="round"
        ...
        <>
          ...
          <div className="relative">
            ...
            >
              ...
              <path
                d={buildPath(rsi, xScale, rsiYScale, visibleStart, visibleEnd)}
                fill="none"
                stroke={COLOR.ink}
                strokeWidth={2}
                strokeLinejoin="round"
        ...
        <>
          ...
          <div className="relative">
            ...
            >
              ...
              {macd.histogram.slice(visibleStart, visibleEnd + 1).map((h, offset) => {
              ...
              <path
                d={buildPath(macd.signalLine, xScale, macdYScale, visibleStart, visibleEnd)}
                fill="none"
                stroke={COLOR.ink}
                strokeWidth={2}
                strokeLinejoin="round"
              ...
              />
              <path
                d={buildPath(macd.macdLine, xScale, macdYScale, visibleStart, visibleEnd)}
                fill="none"
                stroke={COLOR.macdLine}
                strokeWidth={2}
                strokeLinejoin="round"
                strokeLinecap="round"
              />
