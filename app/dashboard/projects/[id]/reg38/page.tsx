...
import { REG38_ITEMS, GOLDEN_THREAD_ITEMS, Reg38ItemDef } from '@/lib/reg38Checklist'
...
export default function Reg38Page() {
  ...
  const allItems = [...REG38_ITEMS, ...GOLDEN_THREAD_ITEMS]
  ...
  return (
    <div className="min-h-screen px-4 py-8">
      <div className="mx-auto max-w-md">
        <PageHeader title="Regulation 38 / Golden Thread" />
        <p className="mt-1 text-sm text-deck-dim">
          {project.name} - fire safety information handover and building safety record-keeping.
        </p>
        {!project.higher_risk_building && (
          <p className="mt-1 text-xs text-deck-mute">
            Not flagged as a Higher-Risk Building - Golden Thread items below are recommended practice, not a legal
            requirement.{' '}
        ...
        <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-deck-dim">Regulation 38</h2>
        <p className="mt-1 text-xs text-deck-dim">
          Fire safety information to hand to the Responsible Person by completion or occupation, whichever is earlier.
        </p>
        <div className="mt-2 space-y-2">{REG38_ITEMS.map(renderItem)}</div>

        <h2 className="mt-6 text-sm font-semibold uppercase tracking-wide text-deck-dim">Golden Thread</h2>
        <p className="mt-1 text-xs text-deck-dim">
          {project.higher_risk_building
        ...
        <div className="mt-2 space-y-2">{GOLDEN_THREAD_ITEMS.map(renderItem)}</div>
      </div>
    </div>
  )
}
